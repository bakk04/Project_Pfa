...
(function () {
	...
	try {
		...
		html.setAttribute("data-theme", savedConfig.theme || defaultTheme);
	} catch {
...
class ThemeCustomizer {
    ...
    // Trigger the global chart refresh
    if (typeof window.refreshChartThemes === 'function') {
      // Small timeout ensures the DOM has updated the [data-theme] styles
      setTimeout(() => {
	...
	updateUI() {
		// Set radio inputs
		document.querySelectorAll("input[name=data-theme]").forEach(radio => {
