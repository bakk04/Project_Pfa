...
class ResidualBlock(nn.Module):
    """
    Bloc résiduel générique : Linear → BN → GELU → Dropout → Linear + skip.
    """
...
class TabNetAttentiveTransformer(nn.Module):
    ...
    def __init__(
        ...
        # Shared layers (partagés entre tous les steps)
        self.shared_fc   = nn.Linear(input_dim, (n_d + n_a), bias=False)
        self.shared_bn   = nn.BatchNorm1d(n_d + n_a, momentum=momentum, eps=epsilon)

        # Step-specific layers
        self.step_fcs  = nn.ModuleList([
            nn.Linear(n_d + n_a, n_d + n_a, bias=False) for _ in range(n_steps)
        ])
        self.step_bns  = nn.ModuleList([
            nn.BatchNorm1d(n_d + n_a, momentum=momentum, eps=epsilon) for _ in range(n_steps)
        ])
        ...
        self.scale = math.sqrt(0.5)  # Facteur de normalisation GLU

    def _sparsemax(self, x: torch.Tensor) -> torch.Tensor:
        """
        Sparsemax : alternative sparse au softmax.
        Produit des masques avec des zéros exacts (pas juste proches de 0).
        Approximation efficace : on utilise une projection sur le simplex.
        """
        # Tri décroissant
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        x : [B, input_dim]
        Retourne :
          - output     : [B, n_d]  embedding TabNet
          - entropy_loss: scalaire  (régularisation d'entropie sur les masques)
        """
        B = x.size(0)
        x_bn = self.initial_bn(x)

        # Priors d'utilisation (initialiser à 1 : toutes les features disponibles)
        prior_scales = torch.ones(B, self.input_dim, device=x.device)
        h_shared     = self.shared_bn(self.shared_fc(x_bn))

        output_agg   = torch.zeros(B, self.n_d, device=x.device)
        entropy_loss = torch.tensor(0.0, device=x.device)

        h_prev = h_shared  # h du step précédent (pour l'attention)

        for step in range(self.n_steps):
            # ----- Attention Transformer -----
            att_input = self.att_bns[step](self.att_fcs[step](h_prev))
            att_input = att_input * prior_scales  # Masquer les features déjà utilisées
            mask      = self._sparsemax(att_input)

            # Mise à jour des priors (features utilisées → pénalisées)
            prior_scales = prior_scales * (self.gamma - mask)

            # ----- Feature Transformer -----
            masked_x  = mask * x_bn
            h_step    = self.shared_bn(self.shared_fc(masked_x))
            h_step    = self.step_bns[step](self.step_fcs[step](h_step))
            # GLU activation (Gated Linear Unit)
            h1, h2    = h_step.chunk(2, dim=-1)
            h_step    = h1 * torch.sigmoid(h2) * self.scale

            # Accumulation de l'output
            output_agg = output_agg + h_step[:, :self.n_d]
            h_prev     = h_step

            # Régularisation par entropie des masques
            entropy_loss += (-mask * torch.log(mask + self.epsilon)).sum(dim=-1).mean()

        entropy_loss = entropy_loss / self.n_steps
        return output_agg, entropy_loss
...
class GatedMultimodalUnit(nn.Module):
    ...
    def forward(
        ...
        # Calcul des portes : g_i = sigmoid(W_g_i * concat_h)
        gate_values = []
        for gate_layer in self.gates:
            g = torch.sigmoid(gate_layer(h_concat))  # [B, 256]
        ...
        # Connexion résiduelle depuis h_clin (branche clinique principale)
        ...
        # Stack des portes pour logging/visualisation
        gates_stacked = torch.stack(gate_values, dim=1)  # [B, 3, 256]

        return z, gates_stacked
