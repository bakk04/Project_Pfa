...
def load_models():
    ...
    meta_cfg_p = os.path.join(PATHS.checkpoints, "meta_learner_config.json")
...
def predict_case(MODELS, clinical_vals, rppg_vals, temporal_vals=None):
    ...
    for feat in NUMERICAL_FEATURES + CATEGORICAL_FEATURES:
        if feat not in df_clin.columns:
            df_clin[feat] = np.nan
    # Mapping for glucose
    ...
    for feat in rppg_prep.feature_names:
        if feat not in df_rppg.columns:
            df_rppg[feat] = np.nan
    ...
    # Meta features in audit script: assuming it matches training (XGB, LGBM, LogReg, GMU, Disagreement)
